...
export function parseSheet(worksheet) {
  ...
  // Detect the real header row as the first "widest" row. This skips leading
  // title/banner rows (e.g. a merged "Footle - UPLOAD" cell in A1) that appear
  // before the actual column headers. Strictly-greater comparison keeps the
  // earliest row on ties, so a normal file's first row stays the header.
  let headerIdx = 0;
  let maxFilled = -1;
  rows.forEach((r, i) => {

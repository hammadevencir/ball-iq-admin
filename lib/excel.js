 ...
 *
 * - Detects the header row as the first row wide enough to be a header,
 *   skipping narrow leading title/banner rows.
...
export function parseSheet(worksheet) {
  ...
  const countFilled = (r) =>
    (r || []).reduce((n, c) => n + (isBlank(c) ? 0 : 1), 0);

  // Detect the real header row. Leading title/banner rows (e.g. a merged
  // "Group Quiz" cell in C1) are narrow — usually a single filled cell — while
  // the header spans most of the sheet's columns. Picking the globally-widest
  // row is wrong: a data row that populates a column the header leaves blank
  // (e.g. an unlabeled numeric column) would then out-width the header and be
  // mistaken for it. Instead, take the FIRST row wide enough to be a header,
  // which skips narrow banners while always preferring the header over any
  // later data row.
  const maxFilled = rows.reduce((m, r) => Math.max(m, countFilled(r)), 0);
  const headerThreshold = Math.max(2, Math.ceil(maxFilled / 2));
  let headerIdx = rows.findIndex((r) => countFilled(r) >= headerThreshold);
